...
def get_account(user_id: int) -> schemas.Account:
    ...
    return schemas.Account(
        ...
        lastUpdated = parser.parse(user_json["lastUpdated"]),
        dateCreated = parser.parse(user_json["dateCreated"]),
...
def update_account(user_id: int, patch_user: schemas.AccountCreate) -> schemas.Account:
    ...
    patch_user.lastUpdated = datetime.now()
    ...
    # format dates to what user ms expects 
    put_data["lastUpdated"] = json.dumps(updated_item.lastUpdated.isoformat(), default=str)[1:-4]+'Z'
    put_data["dateCreated"] = json.dumps(updated_item.dateCreated.isoformat(), default=str)[1:-4]+'Z'
